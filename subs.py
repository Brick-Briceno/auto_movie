...
def txt_to_img(text, font, font_size=48, color=(255, 255, 255),
    ...
    if text[0] in emojis_data.emojis.keys() and len(text) == 1:
        emoji = Image.open(emojis_data.emojis[text])
    ...
    for i, char in enumerate(text[1:]):
        ...
        if char in emojis_data.emojis.keys():
            emoji = Image.open(emojis_data.emojis[char])
            emoji = emoji.resize((int(34*base), int(int(34*base)*(emoji.height/emoji.width))))
            img = merge_with_transparency(img, emoji, (x-int(3*base), int(9-y*base)))
            x += int(base)*26
            continue
        elif not char.isascii():
            print(f"Character {char} U+{ord(char):04x} not found")
...
def make_text(text, font, color="#000", stroke_width=0,
    ...
    base = font_size*(1/32)
    for i, line in enumerate(fit_text(text, 30).split("\n")):
